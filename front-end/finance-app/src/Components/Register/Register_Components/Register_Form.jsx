...
const RegisterForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('/api/register', data, {
      ...
      if (err.response?.data.errors) { setRegister({ ...registerInput, error_list: err.response.data.errors }) };
        ...
        <Slider ref={sliderRef} {...settings}>
          ...
          <div className='etapeone'>
            <div className='inputs_gp'>
              ...
              <span className='err-reg'>{registerInput.error_list.username}</span>
              <input type="password" placeholder='password' name='password' onChange={handleInput} value={registerInput.password} />
              <span className='err-reg'>{registerInput.error_list.password}</span>
              ...
              <span className='err-reg'>{registerInput.error_list.password_confirmation}</span>
            </div>
          </div >
          {/* etape 2 */}
          ...
          <div className='etapetree' >
            <div className='inputs_gp'>
              <div className='gp11'>
                ...
                <span className='err-reg'>{registerInput.error_list.email}</span>
                <input type="text" placeholder='CIN' name='cin' onChange={handleInput} value={registerInput.cin} />
                <span className='err-reg'>{registerInput.error_list.cin}</span>
              </div>
              <div className='gp22'>
                ...
                <span className='err-reg'>{registerInput.error_list.birthday}</span>
                <input type="text" placeholder='Gender' name='gender' onChange={handleInput} value={registerInput.gender} />
                <span className='err-reg'>{registerInput.error_list.gender}</span>
              </div>
            </div>
          </div >
          {/* etape 4 */}
